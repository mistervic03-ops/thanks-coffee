...
| `SCHEDULER_ENABLED` | 선택 | `false` | `true`이면 자동 주간/월간 summary scheduler를 실행한다. Spark PoC 기본 운영은 `false`로 두고 수동 `/summary`를 사용한다. | 예 |
...
운영자가 feed 게시 없이 summary를 확인하려면 `/summary weekly preview`, `/summary monthly preview`, `/summary this-month preview`를 사용한다. 자세한 운영 절차는 `docs/OPERATIONS.md`를 따른다.
