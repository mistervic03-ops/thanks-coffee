...
features:
  ...
  slash_commands:
    ...
    - command: /mocha
      ...
      usage_hint: "delete {recognition_id}, summary weekly|monthly [preview], summary this-month preview, pin"
      should_escape: true
  bot_user:
    display_name: ☕ 모카
    always_online: true
...
`pins:write`는 `/mocha pin`이 전사 공지 채널의 소개 메시지를 Slack `pins.add` API로 pin하기 위해 필요하다. `users:read`는 Slack workspace Admin/Owner를 자동으로 운영자 목록에 포함하고 주간 리마인더 대상 활성 사용자를 조회하기 위해 필요하다.
...
Slack 앱 설정 > Slash Commands에서 `/summary` 커맨드는 제거하고 `/mocha` 커맨드를 추가한다. Description은 `mocha 관리자 명령어`, Usage hint는 `delete {recognition_id}, summary weekly|monthly [preview], summary this-month preview, pin`으로 설정한다.

`/thanks` 또는 `/thanks help`는 일반 사용자용 핵심 사용법과 App Home 안내를 ephemeral message로 보여준다. `/thanks status`, `/thanks received`는 직접 입력 가능한 보조 조회 명령어로 유지하지만 일반 help에는 노출하지 않는다. `/mocha`는 관리자 전용 운영 명령어이며 잘못 입력된 recognition 삭제, 수동 summary 게시/미리보기, 봇 소개 메시지 pin에 사용한다.
...
| `FEED_ENABLED` | 선택 | `true` | recognition과 summary를 전사 공지 채널에 게시할지 결정한다. | 예 |
| `ANNOUNCEMENT_CHANNEL_ID` | 조건부 필수 | 빈 값 | 전사 공지 채널 ID다. `FEED_ENABLED=true`이면 필수이고, `FEED_ENABLED=false`이면 비워둘 수 있다. | 예 |
| `ADMIN_USER_IDS` | 선택 | 빈 값 | `/mocha`를 실행할 수 있는 추가 Slack user ID 목록이다. 쉼표로 구분한다. Slack workspace Admin/Owner는 자동 포함된다. | 예 |
| `SCHEDULER_ENABLED` | 선택 | `false` | `true`이면 자동 주간/월간 summary와 10분 간격 feed 재시도 scheduler를 실행한다. Spark PoC 기본 운영은 `false`로 두고 수동 `/mocha summary`를 사용한다. | 예 |
| `REMINDER_ENABLED` | 선택 | `false` | `true`이면 매주 수요일 10:00 KST에 전사 공지 채널로 주간 감사 리마인더를 게시한다. | 예 |
| `ANTHROPIC_API_KEY` | 조건부 필수 | 빈 값 | 리마인더 문구 생성용 Claude API key다. `REMINDER_ENABLED=true`이면 필수다. | 예 |
| `ANTHROPIC_MODEL` | 선택 | `claude-haiku-4-5-20251001` | 리마인더 문구 생성에 사용할 Claude 모델이다. | 예 |
...
recognition feed와 summary를 게시할 전사 공지 채널에 ☕ 모카를 초대한다.

## 6. 동작 확인 체크리스트

DB 초기화와 `/thanks` ephemeral 응답을 확인한다. `HEALTH_CHECK_ENABLED=true`이면 `/health`와 `/ready`도 함께 확인한다.

운영자가 전사 공지 채널 게시 없이 summary를 확인하려면 `/mocha summary weekly preview`, `/mocha summary monthly preview`, `/mocha summary this-month preview`를 사용한다. 자세한 운영 절차는 `docs/OPERATIONS.md`를 따른다.
