...
주의: 이번 PoC에는 summary 분산락이 없다. `SCHEDULER_ENABLED=true`인 프로세스가 2개 이상 떠 있으면 같은 weekly/monthly summary가 중복 게시될 수 있다. Spark 운영에서는 scheduler를 켠 프로세스를 하나로 제한하거나, 수동 `/summary` 운영을 유지한다.
...
```sql
SELECT id, sender_id, receiver_id, feed_post_status, feed_channel_id, feed_message_ts, created_at
FROM recognition
WHERE feed_post_status IN ('failed', 'pending')
