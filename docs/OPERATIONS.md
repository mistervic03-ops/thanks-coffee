...
- `/mocha summary weekly`: 직전 월요일부터 직전 일요일까지의 요약을 feed 채널에 게시한다. 자동 weekly summary와 같은 기준이며, 실패한 주간 게시를 수동으로 재게시할 때 사용한다.
- `/mocha summary monthly`: 직전 월의 요약을 feed 채널에 게시한다. 자동 monthly summary와 같은 기준이다.
- `/mocha summary weekly preview`: 직전 주 요약을 feed 채널에 게시하지 않고 실행한 운영자에게만 보여준다.
- `/mocha summary monthly preview`: 직전 월 요약을 feed 채널에 게시하지 않고 실행한 운영자에게만 보여준다.
- `/mocha summary this-month preview`: 이번 달 1일부터 현재 날짜까지의 요약을 feed 채널에 게시하지 않고 실행한 운영자에게만 보여준다. 테스트/운영 확인용이며 feed 게시 기능은 없다.
- `/mocha delete {recognition_id}`: 잘못 입력된 단일 recognition을 삭제한다. feed 메시지가 있으면 함께 삭제를 시도한다. feed 삭제 실패 시에도 DB 삭제는 유지되며 앱 로그에 `feed_delete_failed`가 남는다.
...
운영자가 `/mocha delete {recognition_id}`를 실행했는데 feed 메시지 삭제만 실패하면 DB 삭제는 되돌리지 않는다. 필요한 경우 `feed_delete_failed` 로그의 recognition ID와 오류를 확인한 뒤 Slack에서 feed 메시지를 수동 삭제한다.
